...
' ' ')"; PEER_REQUESTED="${PEER_REQUESTED% }"; shift 2 ;;
        ...
        # The LINK cap, and it belongs to the PAIR of hosts rather than to any
...
Peer pairing -- two hosts with NO prior trust (see PAIRING-DESIGN.md):
  --pair                  initiate pairing with a peer (run on the host that
    ...
    --bandwidth=RATE        cap this LINK, in BYTES per second with the usual
                            b/k/M/G suffixes (a 20 Mbps line is 2M). It is
                            recorded in the pairing, so it applies to EVERY
                            relationship with this peer -- which is what "the
                            same wire" means. Omitted: no cap.
...
# THE ENGINE'S set, transcribed rather than approximated: snapsend/snapget accept
# ^[0-9]+[bkKmMgG]?$ and hand it to `mbuffer -r`, which is a BYTE rate. A
# validator stricter than its engine refuses something that works; a looser one
# writes a value the engine rejects every night, in a cron job nobody is
# watching. Both mistakes have been made in this tree already.
if [ -n "$PEER_BANDWIDTH" ]; then
    _bw="$PEER_BANDWIDTH"
    case "$_bw" in *[bkKmMgG]) _bw="${_bw%?}" ;; esac
    case "$_bw" in
        ""|*[!0-9]*)
            echo "--bandwidth='$PEER_BANDWIDTH' is not a byte rate the engines accept (digits, then at most one of b/k/M/G -- e.g. 20M). It is BYTES per second, not bits: a 20 Mbps link is 2M" >&2; exit 2 ;;
    esac
    unset _bw
fi
...
# The three are SUB-MODES of --pair: do_pair dispatches them, and nothing else
# reads the flags. Without --pair they used to fall straight through to the
# default path -- a FULL host deployment (git pull, scripts reinstalled, the
# crontab rewritten) from a command that was meant to draw a file of comments.
# Measured on pve9, 2026-09-03: `deploy.sh --peer=X --draft-config` went
# through Phases 1-8 and moved the checkout under an update hold.
if [ "$PAIR_MODE" -eq 0 ] && [ $(( PEER_ROTATE + PEER_REVOKE_OLD + PEER_DRAFT_CONFIG )) -gt 0 ]; then
    echo "--rotate, --revoke-old and --draft-config are sub-modes of --pair: add --pair --peer=HOST. Without it this would have been a full host deployment, not a pairing step" >&2
    exit 2
fi
...
# ------------------------------------------------------------------------------
# THE SCRIPTS A HOST RUNS ARE FILES IN THE CHECKOUT, NOT HEREDOCS (2026-09-03).
# notify-fail.sh, notify-warn.sh, alert-digest.sh and check-pool-capacity.sh
# used to be generated right here from unquoted heredocs: some 1,350 lines of
# shell in which every runtime '$' had to be written '\$', a backtick inside a
# COMMENT ran a command as root at install time (REV-20260902-133, twice), and
# "current" meant a version marker a human remembered to bump -- or did not,
# and every host kept the old copy while deploy reported it current. They live
# in hostscripts/ now, complete and runnable as they stand, and are installed
# by copying: current means byte-identical to the checkout, so there is
# nothing to expand and nothing to bump. Their shared config/env preamble,
# once a variable interpolated into three of them, is hostscripts/alert-env.sh,
# installed beside them and sourced at run time.
HOST_SCRIPTS_DIR="$_DEPLOY_DIR/hostscripts"
host_script_ensure() {   # <name> <target> <mode> <what is lost without it> <what an old copy gets wrong>
    local name="$1" target="$2" mode="$3" missing="$4" outdated="$5"
    local src="$HOST_SCRIPTS_DIR/$name"
    [ -r "$src" ] || die "cannot read $src -- this checkout is incomplete; $target is installed from it"
    # "Current" is the file itself, not a marker in it: byte-identical to the
    # checkout, and runnable when it is meant to be run.
    local current=0
    if [ -e "$target" ] && cmp -s "$src" "$target"; then
        if [ "$mode" = 0755 ]; then [ -x "$target" ] && current=1; else current=1; fi
    fi
    if [ "$CHECK_ONLY" -eq 1 ]; then
        if [ ! -e "$target" ]; then
            warn "  $target missing -- $missing"
        elif [ "$current" -eq 1 ]; then
            log "  $target present (current)"
        else
            warn "  $target present but not the checkout's copy ($outdated) -- re-run without --check-only to upgrade"
        fi
        return 0
    fi
    if [ "$current" -eq 1 ]; then
        log "$target already current, leaving it alone"
        return 0
    fi
    [ -e "$target" ] && log "$target is not the checkout's copy -- upgrading ($outdated)"
    install -m "$mode" "$src" "$target" || die "could not install $target from $src"
    log "installed $target from hostscripts/$name"
}

ALERT_ENV_FILE="/root/scripts/alert-env.sh"
host_script_ensure alert-env.sh "$ALERT_ENV_FILE" 0644 \
    "notify-fail.sh, notify-warn.sh and alert-digest.sh refuse to start without it" \
    "config/env precedence rule"

NOTIFY_SCRIPT="/root/scripts/notify-fail.sh"
host_script_ensure notify-fail.sh "$NOTIFY_SCRIPT" 0755 \
    "job failures would be silent" \
    "queue path, cooldown, or mails immediately"
...
host_script_ensure notify-warn.sh "$WARN_SCRIPT" 0755 \
    "WARNING monitor lines would error out" \
    "queue path"

DIGEST_SCRIPT="/root/scripts/alert-digest.sh"
host_script_ensure alert-digest.sh "$DIGEST_SCRIPT" 0755 \
    "findings would queue forever and never be seen" \
    "queue path, sections, or WARN-only"
...
host_script_ensure check-pool-capacity.sh "$CAPACITY_SCRIPT" 0755 \
    "no early warning before a pool fills up" \
    "mails on its own instead of joining the daily digest"
...
PEER_SAVED_BANDWIDTH=$PEER_BANDWIDTH
...
' | grep -qx "zfsalert" || warn "  $BACKUP_USER not in group zfsalert -- it could not queue alerts, and lib-cron.sh would refuse to lock its own crontab (REV-20260803-035)"
        ...
        [ -r "/home/$BACKUP_USER/alert-env.sh" ] || warn "  /home/$BACKUP_USER/alert-env.sh missing -- that account's notify scripts refuse to start"
    ...
    # Removing that ordering dependency is the whole point of the merge.
        ...
        # Same two scripts root has, same shared queue, same config -- and the
        # alert-env.sh they source from beside themselves. Only the digest is
        # not duplicated: two digests would mean two mails per host per day.
        for s in alert-env notify-fail notify-warn; do
            if [ -f "/root/scripts/$s.sh" ]; then
                install -o "$USERNAME" -g "$USERNAME" -m "$([ "$s" = alert-env ] && echo 0644 || echo 0755)" "/root/scripts/$s.sh" "$HOMEDIR/$s.sh" \

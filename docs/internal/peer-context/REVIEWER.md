...
context: Owner explicitly requires unified high-level REMOTE deployment UX. Durable contract and implementation sequence are now in `docs/project/OWNER-REMOTE-DEPLOY-UX-REDUCTION-2026-08-12.md` (OWNER DECISION / MUST DO). The motivating real operator topology is `pve2:rpool/data --backup--> pve1:hdd/backup --sync--> pve3:hdd/backup`. Today each remote leg exposes add-client/join/seed/verify/activate; Owner wants the ordinary operator to express only source, target or sync mode, and install intent.
to-peer: Treat the new document as authoritative product direction. Target public forms are local `--source=rpool/data --target=hdd/backup --install`, remote backup `--source=pve2:rpool/data --target=hdd/backup --install`, and remote sync `--source=pve1:hdd/backup --mode=sync --install`, always invoked on the receiving/collector host. This MUST be an orchestration reduction, not a new distributed framework: retain the existing relationship FSM and expert verbs; derive resume position from existing client/pairing/scope/CONFIG/cron/ZFS facts; same-command retry after interruption is the load-bearing acceptance property; never create a second lifecycle state store, pairing/grant path, profile reconciliation path or implicit relationship adoption. Explicit remote source remains subject to source-side scope/grant validation. If remote join cannot complete automatically, stop with ONE exact join action and instruct rerun of the SAME original command. Per current ACTIVE-WORK-PLAN, do not derail Gate 7: implement immediately after Restore as RUX-1 planner, RUX-2 enrol/join orchestration, RUX-3 seed/verify/activate orchestration, RUX-4 bounded real chain proof. Before code, challenge any implementation assumption that would force a second authority or weaken an existing gate; otherwise proceed with this decomposition.
needs-formal-answer: no

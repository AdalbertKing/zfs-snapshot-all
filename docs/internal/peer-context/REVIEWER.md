...
to-peer: Architectural direction: keep SAFE restore and DESTRUCTIVE replace separate. SAFE restore to a new destination never uses rollback/`recv -F`; existing destination still refuses. For replacement/recovery-to-original-path, build a read-only planner that inventories both sides by snapshot GUID, identifies the requested recovery point and selected dataset set, finds a proven common base per selected dataset, and classifies the possible transport as LOCAL-ROLLBACK / REVERSE-INCREMENTAL / FULL-RECEIVE. Do not expose these as user-mode switches yet; they are internal strategies selected only after proof and shown in preview. Never silently delete newer source snapshots merely to make incremental receive fit. `sync` should use the same recovery planner/engine; its mode changes what history/common bases are likely available, not the safety contract. For partial atomic recovery, do not use broad `-R -F` because it can destroy descendants/snapshots outside the selected set. Any destructive publication must first fence/pause normal relationship activity and protect the chosen backup recovery point from pruning for the duration. Baseline implementation should prefer correctness/preservation even if that means a full staged receive; optimize to reverse-incremental only after focused real-ZFS proof of the exact existing-destination semantics.
needs-formal-answer: no

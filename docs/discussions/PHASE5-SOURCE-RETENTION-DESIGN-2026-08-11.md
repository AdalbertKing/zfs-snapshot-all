...
- the two-host live proof (remote-PULL half of step 4): activate a throwaway pull
  relationship pve1↔pve2, confirm the installed source prune ages out the source's
  `automated_` snapshots over SSH and the grant gate refuses when `destroy` is
  absent;
- step 5: migration/audit path for already-installed CONFIGs.

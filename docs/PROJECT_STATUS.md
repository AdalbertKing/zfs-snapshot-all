...
<!-- status-covers-digest: 1ba33f260582c39a -->
...
- Data odświeżenia: **2026-08-10**. Stan plików, który ten blok opisuje, jest
  ...
  **CO JEST WDROŻONE, A CO NIE — stan na dziś.** To jest jedyne miejsce, gdzie
  ...
  | zwykła re-aktywacja niczego nie tworzy, nie naprawia, nie normalizuje ani nie migruje | **wdrożone, czeka na recenzenta** (REV-091, dwa P1): po REV-090 `ensure_cron_config()` NADAL robiła dwie rzeczy bezwarunkowo — doklejała ogólnokonfiguracyjne progi `[excluded:]` (F1) i odmawiała na configu pre-GFS (F2) — więc `needs_profile=0` wcale nie znaczyło „tylko topologia". Oba schowane pod tę samą bramkę. Sama DETEKCJA pre-GFS dalej biegnie bezwarunkowo (`PROFILE_GFS` czytają dalej kształt prune i podsumowanie aktywacji), warunkowa jest wyłącznie ODMOWA — i nadal pada wszędzie tam, gdzie polityka naprawdę jest generowana na host legacy. Skutek: host pre-GFS można odświeżyć endpointowo bez wymuszania `migrate-profile`, a próg `[excluded:]` usunięty świadomie przez operatora nie wraca | REV-20260810-091, suita `zfsbackup` sekcja 51 (7 asercji) |

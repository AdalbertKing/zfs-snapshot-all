...
<!-- status-covers-digest: 7a98208adcc31fd2 -->
...
- **LAB `--draft-config` — WYKONANY, sześć własności OK, dwa defekty
  ...
  `--draft-config`/`--rotate`/`--revoke-old` bez `--pair` spadały na domyślną
  ścieżkę i robiły PEŁNE WDROŻENIE hosta (na pve9 Fazy 1–8, checkout ruszył
  mimo holdu); teraz odmowa przy argumentach, rc=2, przed bramką roota;
  `test/join` +4 (trzy pod-tryby + kontrola, że poprawna forma przechodzi
  bramkę argumentów). **Defekt 2 (naprawiony):** `deploy.sh` wołany wprost

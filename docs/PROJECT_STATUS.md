...
<!-- status-covers-digest: cd0bd45e48bddad3 -->
...
- Data odświeżenia: **2026-08-13**. Ostatnia zmiana zachowania: `cffd1c4`
  (2026-08-13, podgląd strategii odtworzenia dowodzi też ŻYWEGO stanu źródła —
  REV-118 F1). Faza 7 ma na main wycinek 1 (planer read-only), wycinek 2
  ...
  **Wycinek destrukcyjnego zastąpienia nadal NIE istnieje** — strategia jest
  pokazywana, nigdy wykonywana. Stan plików, który ten blok opisuje, jest
  ...
  **CO JEST WDROŻONE, A CO NIE — stan na dziś.** To jest jedyne miejsce, gdzie
  ...
  | restore | **CZĘŚCIOWO wdrożone** (Faza 7): planer read-only, bezpieczne odtworzenie do wyprowadzonego namespace, mapa planera i podgląd domyślnej strategii odtworzenia (`cffd1c4`). Czasownik NISZCZĄCY — zastąpienie oryginalnej ścieżki — **nie istnieje** | wiersz suity `restore` niżej; `OWNER-RECOVERY-DEFAULT-POLICY-2026-08-12.md` |

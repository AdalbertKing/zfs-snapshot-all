...
| Pakiet | Wynik | Zakres |
|---|---|---|
| `impact` | **32/32** | rozwiązywanie grafu testowego + `--verify` na prawdziwym drzewie. +12 (REV-20260807-068, dwie rundy): niezmiennik świeżości `PROJECT_STATUS.md` dotyczy **PROSPEKTYWNEGO COMMITA — indeksu gita** — a nie SHA commita (runda 1) ani drzewa roboczego (runda 2, odrzucona). Runda 1: znacznik nazywał ostatni commit zmieniający zachowanie, czego nie da się sprawdzić przed tym commitem. Runda 2: skrót z drzewa roboczego, a `git commit` zapisuje indeks — zainscenizuj plik, odśwież status tylko w drzewie roboczym, nie inscenizuj go: bramka mówi czysto, commit ląduje nieświeży. Teraz WSZYSTKO, na czym opiera się werdykt, czytane jest z indeksu: blob obserwowany, `PROJECT_STATUS.md` ze znacznikiem oraz `deps.conf`, który DEFINIUJE zbiór obserwowany. Gwarancja: jeśli zaraz po zielonym `--verify` wykonasz zwykły `git commit` bez ruszania indeksu, zacommitowane drzewo spełnia niezmiennik (`commit -a`, `commit <ścieżka>` i `--amend` ruszają indeks i są jawnie poza gwarancją). Każdy przypadek to osobne repozytorium git z KOPIĄ badanego skryptu, więc ta sama konstrukcja uruchamia kontrolę (`IMPACT_UNDER_TEST=`). Jeden przypadek z rundy 2 został ODWRÓCONY: edycja NIEzainscenizowana nie wchodzi do commita, więc nie może brudzić bramki. Kontrola negatywna wobec `41bd774`: **3 asercje padają, w tym decydujący rozjazd indeks/status**, a przypadek z REV-061 nadal przechodzi |
...
| `recursion` | **22/22** | dokładnie JEDNA deklaracja rekursji na wywołanie (Etap 2.2). Na PRAWDZIWYCH silnikach: własnością jest to, że odmowa zapada przy parsowaniu argv, przed jakimkolwiek sprawdzeniem zależności, datasetu czy SSH — dlatego nie potrzeba roota ani ZFS-a. Przypina, że `-r -R` jest odrzucane w OBU kolejnościach — to było prawdą już wcześniej, a kontrakt Etapu 2 twierdził inaczej, więc jest przypięte zamiast zakładane — oraz że powtórzenie TEGO SAMEGO trybu też jest odrzucane, co dokłada 2.2. Powtórzenie jest dziś nieszkodliwe; reguła dotyczy deklaracji, nie jej skutku, bo gdy pojawi się `--recursive=<tryb>` (2.3), pisownia „zwijająca się nieszkodliwie" to dokładnie sposób, w jaki `--recursive=no -r` po cichu stałoby się `-r`. Plus: litera rekursji użyta jako ARGUMENT OPCJI (`-m -r`) to dana, nie deklaracja — klasa błędu, którą REV-054 wyleczyła w generatorze. Kontrola negatywna wobec `a51e09f`: **6 asercji nowej reguły pada, 16 pozostałych przechodzi** |

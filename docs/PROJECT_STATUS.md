...
| `migrate` | **34/34** lokalnie, **36/36** na Linuksie jako konto delegowane (REV-057) | `gen-cron.sh --migrate-recursion`: wykrywanie przez ten sam przebieg opcji co walidator (`-m R-daily_` nietykane, `-Rv 3` rozdzielane), porównanie trójstronne z kontrolą jako pierwszą, zapis transakcyjny. Każdy przypadek odmowy sprawdza sumę kontrolną pliku źródłowego, nie tylko komunikat. D4 (nieudany zapis) wymaga nie-roota — pod rootem SKIP, bo root omija prawa katalogu |

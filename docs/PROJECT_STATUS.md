...
<!-- status-covers-digest: 61468c827c14aba3 -->
...
- Data odświeżenia: **2026-08-09**. Stan plików, który ten blok opisuje, jest
  ...
  **CO JEST WDROŻONE, A CO NIE — stan na dziś.** To jest jedyne miejsce, gdzie
  ...
  | Etap 5 — RUNTIME profilu (`zfs-backup.sh` czyta profil zamiast zaszytych szablonów) | **wdrożony** (B1 krok 2): `ensure_cron_config` i `emit_client_sections` czytają wyrenderowany profil, pre-GFS **odmawia** zamiast konwertować; suita `zfsbackup` 310/310, kontrola negatywna 14 pada | `docs/design/slice-b1-plan.md` |
  ...
  | preflight nakładania pokrycia (create-only preset odmawia relacji, której ścieżka jest rodzicem/dzieckiem/dokładnym trafieniem cudzego pokrycia; fail-closed też na rekordzie nie do odczytu/parsowania) | **wdrożony**, kod: `coverage_conflicts`/`assert_no_coverage_overlap` w `zfs-backup.sh`; suita `zfsbackup` sekcje 45/46 | REV-20260809-083/084; wymagany dowód na żywo (nadpisanie na prawdziwym hoście + odczyt zwrotny) NIEwykonany, patrz odpowiedzi REV-083/084 |

...
<!-- status-covers-digest: 34a24441b1cc9b58 -->
...
- Data odświeżenia: **2026-08-11**. Stan plików, który ten blok opisuje, jest
  ...
  **CO JEST WDROŻONE, A CO NIE — stan na dziś.** To jest jedyne miejsce, gdzie
  ...
  | domyślny backup ogranicza retencję ŹRÓDŁA (nie zostawia snapshotów źródła bez ograniczeń) | **CZĘŚCIOWO — REV-102 OPEN**: krok 2 (local PUSH `cmd_local_backup`) wdrożony — osobna `[prune:<root>]` z tej samej drabiny, non-recursive, tylko `automated_`; grant-guard `assert_source_prune_grant()` (fail-closed na braku `destroy`/błędzie ssh, bez poszerzania) **wylądował + test** (sekcja 55). Odłożone: krok 3 (remote PULL emisja w `emit_client_sections`, bramkowana grant-guardem), krok 4 (dowód na żywym ZFS — dowód destrukcyjny wymaga uruchomienia **poza auto mode**), krok 5 (migracja istniejących CONFIG) | `docs/discussions/PHASE5-SOURCE-RETENTION-DESIGN-2026-08-11.md`; suity `localbackup` 35/35, `zfsbackup` 368/368 |

...
<!-- status-covers-digest: e6bee71ee4d3ba3a -->
...
- **REV-20260907-137: `save-profile` pyta prawdziwy generator, zanim opublikuje (2026-09-07, IMPLEMENTED → recenzent, `4e6556e`).**
  ...
  **własne** czytanie profilu przez `zfs-backup.sh`; żadna nie pytała
  `gen-cron.sh`, czy przyjmie wyrenderowaną politykę, a wartość, którą render
  tylko przepisuje (`send_schedule = not a cron`), przechodziła obie, była
  publikowana, katalog mówił `valid:true`, a `add-client` odmawiał. Z
  `--force` zastępowało to działający profil operatora. Odtworzone na
  `6498652` przed zmianą: rc=0, plik zapisany.
  **Bramka 3**: `profile_downstream_candidate` składa relację w kształcie
  `emit_client_sections` (sekcje `[template:]` i `[excluded:]` dosłownie, jeden
  `[dataset:]` z `use_template` profilu, `[prune:]` tylko gdy profil deklaruje
  drabinę) i **prawdziwy** `gen-cron.sh -c` ją renderuje; odmowa cytuje słowa
  generatora i pada **przed** stage'owaniem, więc cel nie powstaje, a plik pod
  `--force` zostaje bajt w bajt. Jeden składacz dla bramki i kontroli — żadnej
  kopii reguł generatora. Zmierzone: `not a cron` → odmowa ze zdaniem o
  pięciu polach; `7 * * * *` → zapis + „gen-cron.sh accepted"; `--force` ze
  złą wartością → sha256 bez zmian; `monitor_warn=soon` → „invalid duration";
  **`monitor_warn=30m` na szczeblu godzinowym → odmowa z reguły kadencji
  generatora** (poprawna składniowo wartość, której żadna przepisana lista nie
  złapie — kryterium 5 recenzji); 16/16 profili fabrycznych przechodzi bez
  zmian, ~1,6 s na zapis. Sekcja `saveprof`: 345/0, +7 asercji; kontrola
  negatywna na `6498652`: dokładnie pięć nowych pada. **Nie zmienione, zgłoszone
...
- **Powtórzone domknięcie recenzji jest transakcją, nie wcześniejszym
  ...
  powtórzeniu opublikowanego już domknięcia drukował „nothing to do" i wracał
  z zerem, nie strzegąc, nie regenerując, nie weryfikując i nie potwierdzając
  niczego. Ta gałąź jest osiągalna **tylko** wtedy, gdy wszystkie fakty
  kanoniczne są poprawne — więc jedyne, czego nigdy nie sprawdzała, to jedyne,
  po co operator powtarza `close`: czy widoki WYPROWADZANE nadal się zgadzają.
  `REVIEW_LEDGER.md` i `OPEN-THREADS.md` są generowane, więc to dokładnie one
  padają ofiarą częściowej edycji ręcznej, rozwiązania konfliktu albo przerwanej
  publikacji, gdy artefakty pozostają nienaruszone. Recenzent odtworzył to na
  prawdziwym domknięciu REV-20260904-134: `rc=0`, a `TAMPERED` dalej ostatnią
  linią ledgera. Powtórzenie wykonuje teraz tę samą transakcję co powtórzenie
  akceptacji: `tx_guard` na obu widokach, regeneracja i weryfikacja przez
  testowaną ścieżkę w procesie potomnym, ponowne potwierdzenie
  `--expected-parent`, komunikat „no write needed; derived views verified".
  Wycofanie nie potrzebowało nowego kodu — `tx_guard` plus `tx_die` odtwarzają
  każdy strzeżony bajt. Przy okazji sprostowana przesłanka recenzji: powtórzenie
  akceptacji, nazwane w niej „już poprawnym", potwierdzenia rodzica **nie**
  robiło; skoro powtórzenie może teraz pisać (naprawia widoki), potwierdzenie
  dodano po obu stronach zamiast czynić domknięcie surowszym niż ścieżka, którą
  ma naśladować. `reviewctl` **96/0**; kontrola negatywna wobec `e9a97bb`:
  93/3, padają dokładnie trzy nowe asercje przy przechodzącej kontroli
  „domknięcie naprawdę się wydarzyło".
...
- **Rodzina `delsnaps.sh`/`lib-zfs-snap.sh` zmierzona: jedna kopia, dwa
  ...
  `destroy_one` i `emit_stats` to NIE kopie, tylko homonimy o innych
  sygnaturach i innej robocie: `delsnaps.sh` kasuje przez `run_zfs` z hostem
  zdalnym i raportuje `deleted`/`kept`, biblioteka kasuje lokalnie i raportuje
  `resumed` transferu — komentarz nad `emit_stats` w `delsnaps.sh` mówi to
  wprost. `json_escape` to jedyna prawdziwa kopia („kept as a separate copy
  here", bo `delsnaps.sh` jest samodzielny i niczego nie sourcuje), oba piszą
  ten sam log JSON-lines, więc escape nauczony po jednej stronie to log, który
  digest parsuje w jednej linii i dławi się w następnej. `test/twins` sekcja G
  (+3): `json_escape` identyczne po normalizacji, dwa homonimy NIE identyczne
  (gdyby się zrównały, ktoś wkleił kopię albo jedna strona straciła swoją
  robotę). Kontrakt `json-escape` w `deps.conf`, `twins` w suitach obu plików.
  Kontrola ujemna na kopii `delsnaps.sh` z dodatkową linią w `json_escape`:
  dokładnie G. Oba pliki zamrożone, nietknięte — pomiar, nie scalenie.

- **Bliźniaki `deploy.sh`/`update-control.sh` pod pomiarem (2026-09-03,
  „2.").** `deploy.sh` nosi awaryjne kopie dziewięciu funkcji kontrolera
  aktualizacji (bootstrap, zanim kontroler jest wdrożony albo gdy go usunięto,
  REV-001 F2); notka w `deps.conf` mówiła „w kroku RĘCZNIE, bez krawędzi
  source" i nic tego nie mierzyło. Zmierzone: sześć identycznych, dwie
  (`do_rollback`, `do_self_update`) różne tylko kontynuacjami linii i tym, jaki
  program nazywa podpowiedź (`$SELF` vs `bash $REPO_DIR/deploy.sh`, bo kopia
  awaryjna nie ma „siebie"), `emergency_disable` różne ZACHOWANIEM celowo:
  kontroler ma dodatkowy stopień `chmod 000 $SELF`, `deploy.sh` go nie ma, bo
  jest narzędziem wdrożenia całej floty. `test/selfupdate` sekcja 28 (+14):
  zbiór wyprowadzany z obu plików (co najmniej dziewięć, `log`/`warn`/`die`
  poza kontraktem jako własny głos każdego programu), osiem musi być
  IDENTYCZNYCH po normalizacji (komentarze, kontynuacje, białe znaki) i jednej
  udokumentowanej podstawie w podpowiedzi, `emergency_disable` ma dwa fakty
  strukturalne (kopia awaryjna nigdy nie chmoduje `deploy.sh`; kontroler
  wyłącza SIEBIE, zanim ruszy crontab) i pin per strona w
  `controller-twins.sha256` (`--bless-twins` po przeglądzie, jak w `twins`).
  Kontrola ujemna na kopii `deploy.sh` ze zmienionym słowem w `do_rollback` i
  w `emergency_disable`: dokładnie 28b `do_rollback` i 28e „zmienione w
  deploy.sh ONLY". Bez zmiany produktu.

- **LAB `--draft-config` — WYKONANY, sześć własności OK, dwa defekty
  produktu naprawione, dwie decyzje właściciela (pve9↔pve10, 2026-09-03/04,
  PR #317 wchłonięty tą gałęzią).** Draft `pull`: dokładnie zadeklarowane i
  obecne korzenie, liczba potomków, `recursive = flat` tylko przy potomkach,
  `-K` na klucz KONTA ZADAŃ, `-k` obecne, `-p` nieobecne przy 22; nieistniejący
  pod `UWAGA`, nieobjęty z podpowiedzią `--pair … --peer-datasets=`, głębsze
  policzone; drugi przebieg nadpisuje identyczną treścią, crontaby i
  `/etc/zfs-snapshot-all` bez zmian; rola `push` z lokalnego `zfs list`;
  przed `--join` odmowa bez pliku (tylko w roli `pull`, push listuje lokalnie).
  Krok 0: zero śladów użycia na siedmiu hostach, nic w produkcie jej nie woła
  — mimo to rekomendacja wykonawcy: zostawić, bo ścieżka niskopoziomowa jest w
  użyciu, gdy `--join-remotely` nie pasuje. **Defekt 1 (naprawiony
  przez wątek floty, PR #318, wpis wyżej):** `--draft-config`/`--rotate`/
  ...
  na rzecz tamtej; kontrola pozytywna tamtej suity NIGDY nie biegnie jako root: za bramką
  argumentów prawdziwy `--pair` robi Fazę 1 i 2 wdrożenia (w tym `git fetch`)
  ZANIM sparuje, więc jako root na checkoucie dewelopera ruszał repozytorium,
  które suita właśnie czyta (zmierzone 2026-09-03: `FETCH_HEAD` pod biegnącą
  suitą); jako root kontrola idzie przez `su nobody`, a bez takiego konta
  zgłasza SKIP z powodem (47/47 tu, CI bez roota bez zmian). **Defekt 2 (naprawiony tu):** `deploy.sh` wołany wprost
  ...
  **Decyzja właściciela 1 — PODJĘTA i wdrożona przez wątek floty (PR
  ...
  Trzy błędy runbooka poprawione, E34 (R8 drugi raz z rzędu: flaga istniała,
  forma komendy nie; R1: istnienie to nie osiągalność).

- **`test/twins` wyprowadza zbiór bliźniaków z obu silników zamiast trzymać
  go ręcznie (2026-09-03, „Bierz punkt 1").** Lista `TWINS` mówiła o sobie
  „wyczerpująca, nie kuratorowana" i miała 12 nazw, a silniki dzieliły 15:
  `announce_transfer_size`, `human_bytes` i `validate_subtree` doszły do obu po
  2026-08-20 i nic ich nie pilnowało — pierwszą z nich zmieniłem tego dnia w
  obu silnikach ręcznie (PR #306), bez pomiaru. Ta sama lekcja co `OPTSTRING`
  z 2026-08-31: ręczna lista drży po cichu, więc zbiór to teraz przecięcie
  funkcji zdefiniowanych w kolumnie 0 w obu plikach (`twin_names`/`comm -12`),
  sekcja C robi z nowej wspólnej nazwy czerwień z nazwą, dopóki ktoś jej nie
  przejrzy i nie przyblessuje, a A0 pilnuje, że wyprowadzenie znalazło co
  najmniej 12 (puste nie przejdzie jako „nic do pilnowania"). Trzy brakujące
  przejrzane i przypięte: dwie identyczne po obu stronach, `validate_subtree`
  to lustro (push listuje źródło lokalnie i cel przez ssh, pull odwrotnie),
  nie dryf. Przed blessem dokładnie te trzy czerwone (63/3), po — 69/69 (A, B i C po jednej asercji na każdą z trzech).

- **`product_range` w harnessie: wycinek produktu między kotwicami kończy
  suitę, gdy kotwicy nie ma (2026-09-03, „Bierz product_range").** Po
  `product_fn` (E30) zostały w suitach wycinki `sed -n '/start/,/end/p'` i
  `awk` po zakresach, które nie są funkcjami: blok zmiennych `PEER_STATE_DIR…`
  w `draftscope`, rodzina `--pause` w `pause`, stanza logrotate w
  `zfsbackup`, strażnik `_mf_user_path` w `linkfields`, `local _prev_state` i
  `case "${STATE:-}"` w `rerun`. `sed` na brakującej kotwicy startowej oddaje
  pusty tekst, a na brakującej końcowej — resztę pliku do EOF, oba z `rc=0`
  (kontrola H16). `product_range <plik> <start-ERE> <end-ERE>` bierze PIERWSZY
  przebieg start..koniec włącznie (awk, wzorce przez `ENVIRON`, bo `-v`
  przetwarza sekwencje `\` i literalny dolar stałby się kotwicą końca
  linii) i na braku którejkolwiek kotwicy robi to samo co `product_fn`:
  FATAL z nazwą kotwicy i pliku, `kill -TERM` suity spod podstawienia. Sześć
  miejsc przepisanych; przy okazji 20 wycinków w kształcie funkcji, które
  wciąż szły przez `awk`/`sed` (linkfields ×5, localbackup ×2, moveclient,
  pause, pairgate, quiesce ×3, quiescehelper ×2, restore, subtree,
  zfsbackup ×8), przeszły na `product_fn`, a sześć suit, które harnessu nie
  sourcowały (`pause`, `quiesce`, `quiescehelper`, `restore`, `subtree`,
  `localbackup`), sourcuje go z WŁASNEGO checkoutu (nie z drzewa pod testem,
  które w kontroli ujemnej może być starsze). Zostają celowo: `rerun` 230
  (trzy kotwice: koniec liczy się dopiero po znaczniku pośrednim),
  `quiescehelper` 428 (rodzina `quiesce_*`, wszystkie funkcje naraz — `sed`
  drukuje każdy przebieg, `product_range` pierwszy), `harness` 125
  (porównuje oba mechanizmy) i wycinki po plikach TESTOWYCH (bloki crontaba,
  stanzy `[dataset:]` — tam brak bloku bywa asercją). `test/harness` +6
  (H12–H17: pierwszy przebieg, jednoliniowy zakres, FATAL na starcie i na
  końcu spod `eval "$(…)"`, kontrola `sed` do EOF, literalny dolar/nawias w
  wzorcu). Trzy suity wywołały helper przed sourcowaniem harnessu (błąd
  własny, złapany przez ich pierwszy przebieg: `command not found`), więc
  `zfsbackup`, `linkfields` i `rerun` sourcują go teraz na górze. Jedna
  realna różnica semantyki złapana przez `zfsbackup`: stanza logrotate w
  `deploy.sh` występuje DWA razy za tym samym markerem (root, potem konto),
  `sed` drukował oba przebiegi, `product_range` bierze pierwszy — miejsce
  zakotwiczone na własnej linii ścieżki konta. E33 (R5): suita edytowana pod
  biegnącym przebiegiem, dwa przebiegi w jednym logu.

- **Detektor martwych funkcji jako suita repo; dwie martwe funkcje produktu
  usunięte (2026-09-03, „Bierz punkt 1 z 3").** Ten sam skan, który dzień
  wcześniej złapał `peer_label`, przepuszczony przez 749 funkcji produktu
  znalazł dwie kolejne, obie UDOKUMENTOWANE jako celowe: (1) `config_datasets`
  w `zfs-backup.sh`, zostawiona 2026-08-20 z komentarzem, że planer
  `assert_no_overlapping_policy` powtarza jej konwencję inline — planera o tej
  nazwie nie ma w żadnej rewizji, a jedyna gramatyka listy to
  `dataset_list_split` w `lib-scope.sh`, pilnowana w `test/scope`; usunięta
  wraz z sekcją 29 `test/zfsbackup` (−3), która pinowała kopię; (2)
  `restore_replace_internal` w `zfs-restore.sh`, starszy wrapper
  jednodatasetowy silnika niszczącego, bez callera, trzymany przy życiu przez
  52 wywołania w `test/restore` — nagłówek pliku wciąż głosił „NO public
  door, gramatyka i grant to decyzje właściciela w toku", nieprawda od
  2026-08-30; usunięty, nagłówek przepisany do stanu drzewa, bateria woła
  `restore_replace_pair` wprost po rozwiązaniu pary przez `restore_relations`
  (ten sam autorytet co `--plan`), trzy bramki wyłącznie wrappera odeszły
  (297/297). Nowa suita `test/deadcode` (7/7): każda funkcja produktu musi być
  nazwana przez inną linię produktu poza komentarzami, testy nie liczą się
  jako callery; detektor sprawdzony na fixture, allowlista z obowiązkowym
  powodem i kontrolą, że nazwana funkcja istnieje. Kontrola ujemna wobec
  `caf8f27`: dokładnie te dwie. Przy okazji plan: cztery linie Faz 2/3.5/5/6
  mówiły „unreviewed direct-main delivery pending a REV", a `DELIVERIES.md`
  nosi ich `reviewed-by` (REV-088, -093, -097, -094) — nazwane.
...
- **`clean-relationships.sh` nosiło martwą kopię `peer_label` od pierwszego
  ...
  niespójny stan pakietu") i w żadnej rewizji pliku żadna linia jej nie
  wołała: narzędzie czyta każdą tożsamość z dysku taką, jaka jest, i nigdy nie
  wyprowadza etykiety samo. Kopia, której nikt nie używa, to nie lustro, tylko
  drugi egzemplarz czekający na rozjazd. Skasowana; `clean-relationships.sh`
  nie sourcuje `lib-pairing.sh`, bo nie ma czego z niej brać (notka w
  `deps.conf` przepisana). `test/cleanrel` +2: brak transformacji `tr -c` w
  jakiejkolwiek pisowni oraz detektor martwych funkcji — każda funkcja
  zdefiniowana w pliku musi być nazwana przez inną jego linię, z jednym
  jawnym wyjątkiem `release_orphaned_holds`, którą własny komentarz oznacza
  jako „unreachable by design" (punkt wpięcia trasy dopuszczonej przez
  recenzenta). Kontrola ujemna wobec `main`: 88/2, obie nowe asercje z nazwą
  `peer_label`. Druga kopia w tym pliku, `HOLD_TAG` z `lib-zfs-snap.sh`, jest
  ŻYWA i pilnowana kontraktem `hold-tag` w `deps.conf` — zostaje. Przy
  okazji: komentarz sekcji 2 `test/zfsbackup` mówił „no source edge" o
  `peer_label`, nieprawda od PR #300 — przepisany (asercja bez zmian: literał
  jest transformacją, którą powstały ISTNIEJĄCE etykiety floty). Sekcja 8 tego
  dokumentu mówiła „obecnie bezpośrednio w `main`" — nieprawda od 2026-08-14,
  przepisana na gałąź + PR + „Scal". `test/gh-api.sh` miał w indeksie tryb
  100644, więc `./test/gh-api.sh` z `CLAUDE.md` odmawiał „Permission denied"
  (zmierzone dziś); bramka `--verify` pilnuje tylko skryptów uruchamianych na
  hostach, stąd przeszło — `git update-index --chmod=+x`.

- **Gate 7 ZADEKLAROWANY przez właściciela (2026-09-03).** „Produkt umie
  zrobić kopię i ją odtworzyć bezpieczną ścieżką" — na liście faktów z
  `ACTIVE-WORK-PLAN.md` (sekcja „Gate 7 — what the facts said") przy `main`
  `abfda49`: create/install przez Gate 5 i Fazę 6, restore bezpieczny
  dowiedziony na wdrożonym kodzie 2026-08-15, restore niszczący za grantem i
  zmierzonym zbiorem strat, dowiedziony w każdej udokumentowanej formie na parze
  labowej (pełny lab 2026-08-31 @ `bb5b6af`), wszystkie REV Fazy 7 zamknięte.
  Sekwencja z planu jest wykonana do końca; dalej są udogodnienia pod realną
  potrzebę i otwarta lista właściciela, nie kolejna bramka. Tylko dokumenty
  (`ACTIVE-WORK-PLAN.md` Gate 7 REACHED, ten dokument).

- **Sześć `eval` w zamrożonych silnikach usunięte — odmrożenie właściciela
  (2026-09-03, „odmrożenie silników pod sześć eval").** Po stronie ZFS nic się
  nie zmienia: ten sam `zfs send -nP` na sucho, ten sam `zfs set
  canmount=noauto` po tej samej liście, te same trzy próbki w sondzie
  autotune. Zmienia się, JAK powłoka silnika je uruchamia, trzy mechanizmy:
  (1) `announce_transfer_size`, gałąź lokalna, oba silniki (2 miejsca): tekst
  próby na sucho szedł do `eval`; teraz jest dzielony na słowa tym samym
  `IFS=' ' read -r -a`, którym `transfer_data` dzieli prawdziwy send trzy
  linie niżej, i wykonywany jako argv — próba nie może uruchomić niczego,
  czego nie uruchomiłby send; nazwa snapshotu ze składnią powłoki dociera do
  `zfs` jako nazwa (pod `eval` była poleceniem); gałąź zdalna bez zmian,
  tekst przez ssh jest tekstem z konieczności. (2) `canmount=noauto` po
  odebranym poddrzewie, `snapsend.sh` (1 miejsce): jeden łańcuch służył obu
  stronom — tekst dla ssh, `eval` lokalnie; teraz para
  `canmount_noauto_subtree` (kod, lokalnie) i `canmount_noauto_subtree_cmd`
  (ten sam one-liner jako tekst, zdalnie), a test pinuje, że obie postaci dają
  identyczne wywołania `zfs set` na tej samej liście i że tekst jest bajt w
  bajt starym. (3) `tune_probe_stream`, `lib-zfs-snap.sh` (3 miejsca):
  snippet po stronie danych budował próbkowany send jako łańcuch `H` i
  `eval`-ował go trzy razy (rozgrzewka, surowe, po kompresji); teraz definiuje
  funkcję `h()` i woła ją trzy razy, nazwa snapshotu rozwijana raz, przez
  powłokę snippetu, jako jedno słowo. Suita `test/evalfree` (nowa, 16/16):
  licznik słowa `eval` poza komentarzami w pięciu zamrożonych plikach = 0;
  `announce_transfer_size` wycięte z każdego silnika, pod pseudo-tty, na
  stubie `zfs` zapisującym argv — tekst sendu niesie `$(touch MARKER)`,
  zapowiedź się pojawia, stub dostał dosłowny tekst jako argument, marker nie
  powstał (na starym kodzie powstawał); para canmount na liście trzech
  datasetów (jeden ze spacją w nazwie); snippet sondy ma `h()` i nie ma
  `eval`. Kontrola ujemna wobec `main` (`b6e593a`): licznik, marker/argv
  zapowiedzi i FATAL na braku helpera canmount — dokładnie dyskryminatory.
  `twins` przeblessowane po przeczytaniu diffu (ruszyło się tylko
...
- **`cron2conf.sh` czyta linie `snapget.sh` — crontab kolektora jest wreszcie
  ...
  `gen-cron.sh` przyjmowała tylko `snapsend.sh -m "…"` i `delsnaps.sh`;
  pierwsza linia pull kończyła bieg `die "unrecognized job line"`, czyli dla
  hosta, który ciągnie kopie z innych maszyn, narzędzie do odzyskania
  zgubionego configu nie działało w ogóle. Teraz `parse_get_cmd` (lustro
  `parse_send_cmd`: `snapget.sh -m "PREFIX" [flagi] "ZDALNY" ["BAZA"]`),
  osobna lista encji `PULL_E` i `build_pull_sections`: sekcja
  `[dataset:<baza>/<nazwa zdalna>]` (albo sama nazwa zdalna, gdy `gen-cron.sh`
  nie wypisał bazy) z `src = <zdalny>`, flagi wracają dosłownie (`-A`, `-q`,
  `-L` w środku — `gen-cron.sh` nie dubluje tokenu, który już jest), a brak
  `-A` przy transferze zdalnym daje `autotune = no`, bo inaczej round-trip
  dorabia flagę; `-r`/`-R` idą przez ten sam `split_recursion` co push.
  Fixture `test/cron2conf/fixtures/pull.crontab`: sześć kształtów pull, które
  `gen-cron.sh` naprawdę emituje (bez bazy, bez autotune, nazwa sync-mode,
  `pair_label` z kluczem, `quiesce`, `flat`) plus jeden push; round-trip
  przechodzi na gałęzi (`cron2conf` 29/0), a na drzewie sprzed zmiany pada
  dokładnie na tym fixture (28/1, „exited 1"). Poza zakresem: pull w
  nawiasie nośnika wymiennego (`media_bracket` wokół `snapget.sh`) —
  `unwrap_media_bracket` dalej zna tylko `snapsend.sh`. Przy okazji
  zmierzone wszystkie cztery „znane luki" z sierpnia (sekcja 6): zostaje
  tylko (2), labowa.

- **Plan produktu i tabela wdrożenia mówiły o restore z sierpnia (2026-09-03,
  poprawione).** `ACTIVE-WORK-PLAN.md` Faza 7 stała na „delivered direct-main
  pending the REV-120/121 verdicts" i „public cross-host CLI follows under
  R-025" (oba dawno zamknięte i wdrożone), a wiersz `restore` w tabeli „co
  jest wdrożone" mówił „publicznej gramatyki nadal nie ma, funkcja
  nieosiągalna z CLI" — nieprawda od 2026-08-30. Sekcja statusu Fazy 7
  PRZEPISANA do stanu drzewa: co dostarczono krok po kroku, że krok 5
  („osobny czasownik + obowiązkowy snapshot") wszedł w innym kształcie
  DECYZJĄ właściciela (grant na maszynie zagrożonej + zmierzony zbiór strat
  ze snapshotu technicznego), cztery dokumenty gramatyki, polityka D+B,
  dowody na żywo, wszystkie REV Fazy 7 zamknięte, suity, stan wdrożenia — i
  osobna lista „Gate 7 — what the facts say" pod deklarację właściciela,
  której implementer nie składa. Wiersz `restore` w tabeli wdrożenia
  przepisany tak samo. Tylko dokumenty, kod nietknięty.
...
- **Każde wycięcie funkcji produktu po kotwicy idzie przez `product_fn`, a
  ...
  bez decyzji).** Osiem suit (`restoregrant`, `alertmail`, `joinremote`,
  `moveclient`, `pairgate`, `runsuffix`, `join`, `joinmanifest`) nadal samo
  pisało `eval "$(sed -n '/^f() {/,/^}/p' plik)"` — 24 miejsca; trzy z nich
  sprawdzały potem `declare -F`, dwie miały asercję „da się wyciąć", reszta
  nic: przestawiona kotwica dawała pustą funkcję i suita mierzyła stub albo
  `command not found` zamiast produktu. Wszystkie idą teraz przez
  `product_fn` z `test/harness.sh`; w drzewie nie ma już ani jednego
  ręcznego wycięcia poza suitą, która pinuje sam mechanizm. **Wada
  znaleziona przy adopcji:** `product_fn` robił `exit 1` przy braku kotwicy,
  ale każdy wołający owija go w `$( )`, więc `exit` kończył tylko
  podstawienie, `eval` dostawał pusty tekst i suita szła dalej — dokładnie
  ten cichy kształt, który helper miał zastępować. Teraz FATAL nazywa
  funkcję i plik, a gdy `$BASHPID != $$`, kończy powłokę suity
  (`kill -TERM $$`; trap EXIT sprząta). Nowa suita **`harness`** (13
  asercji, `needs = nothing`, w macierzy CI przez `deps.conf`): wycięcie
  jednolinijkowe i wielolinijkowe, brak kotwicy z podstawienia / z
  podpowłoki w funkcji w podstawieniu / z poziomu głównego, trap EXIT,
  kontrola ujemna starego kształtu (leci dalej z rc=0) oraz `product_libs`
  (drzewo pełne, drzewo starsze bez bibliotek, `$REPO`, brak obu → odmowa
  po nazwie). Asercje „da się wyciąć" (`join` L0, `joinmanifest`
  pairmode/0) usunięte, bo nie mogły już paść. Kontrole ujemne adopcji:
  `ZB=/dev/null ./test/moveclient/run.sh` → FATAL, rc=143, zero asercji;
  to samo na `main` → 41 linii `command not found`/FAIL i suita kończy
  8/33; `DEPLOY_SRC=/dev/null ./test/joinmanifest/run.sh` → FATAL,
  rc=143. Suity lokalnie: `harness` 13/0, `alertmail` 79/0, `joinremote`
  12/0, `moveclient` 41/0, `runsuffix` 15/0, `pairgate` 84/0, `join` 93/0
  (samodzielnie), `joinmanifest` 32/0, `restoregrant` 105/3 (trzy asercje
  wymagające konta bez roota, w CI zielone), `zfsbackup --section noeval`
  207/0.

- **`lib-pairing.sh` — układ stanu parowania zdefiniowany raz, nie w dwóch
  programach (2026-09-03, po punkcie 3 z listy redukcji).** `deploy.sh
  --pair/--join` PISZE układ katalogu peera (manifest, plik zakresu, sidecar
  sha256 grantu, klucz zadania, przypięty klucz hosta), a `zfs-backup.sh
  add-client/activate-client` go CZYTA — bez żadnego wywołania między
  programami. Każdy niósł własną kopię sześciu formuł (`peer_label`,
  `peer_manifest_path`, `peer_scope_path`, `peer_scope_granted_hash_path`,
  `local_keyfile_path`, `local_knownhosts_path`) „utrzymywaną w zgodzie
  celowo, bo nie ma krawędzi source do zadeklarowania"; dwie identyczne
  bajt w bajt, cztery liczące tę samą ścieżkę w innym kształcie (`deploy.sh`
  z globala `$PEER_HOST`, `zfs-backup.sh` z argumentu), a suita `zfsbackup`
  pinowała parzystość jednej z nich, tej, która raz wypadła z obu wyliczeń
  (audyt C1). Teraz jedna definicja w `lib-pairing.sh`, sourcowana przez
  `lib-backup-common.sh` (dla `zfs-backup.sh` i `zfs-restore.sh`) i przez
  `deploy.sh` (z tą samą odmową niekompletnego checkoutu co `lib-record.sh`);
  `peer_label` bierze adres jako argument, jedyny wołający po `--peer`
  (`do_pair`) podaje `$PEER_HOST`. Deklaracja w `deps.conf`, allowlista w
  `.gitignore`, `product_libs` w `test/harness.sh` ładuje ją z drzewa pod
  testem. **Jedna zmiana kontraktu:** `PEER_STATE_DIR` jest nadpisywalne ze
  środowiska także w `zfs-backup.sh` (w `deploy.sh` było; `zfs-backup.sh`
  miało ścieżkę na sztywno) — ta sama forma co `CLIENTS_DIR`. `PEER_KEY_DIR`
  bez zmian, na sztywno w obu. `clean-relationships.sh` zachowuje własną,
  jednolinijkową kopię `peer_label` — jego komentarz wprost odmawia
  sourcowania czegokolwiek na hoście o niespójnym stanie pakietu; zmiana tej
  decyzji to osobna sprawa. Dyskryminator (`zfsbackup` 3b, przepisane):
  żaden z dwóch programów nie definiuje żadnej z sześciu (na `main`: 12
  kopii), biblioteka definiuje każdą dokładnie raz; do tego pin kształtów
  ścieżek z adresu zawierającego znaki niedozwolone w nazwie pliku
  (`pve2.lab:2222/x` → `pve2.lab-2222-x`) i pin, że `do_pair` podaje
  `$PEER_HOST`. `draftscope` wycina zakres `deploy.sh` od jednej z dwóch
  kotwic: na starszym drzewie od deklaracji `PEER_STATE_DIR` (zakres niesie
  kopie), na bieżącym od nagłówka `remote_scope_stage` (helpery daje
  `product_libs`) — kontrola ujemna `DEPLOY_SRC=<main>` 36/0 po obu stronach.
  Suity lokalnie: `draftscope` 36/0, `joinmanifest` 33/0, `rux` 45/0,
  `linkfields` 49/0, `rerun` 21/0, `pause` 83/0, `restore` 300/0, `impact`
  63/0, `join` 94/0 (samodzielnie), `zfsbackup` pełne 559/0. Kontrola ujemna
  `zfsbackup` z `ZFSBACKUP=<main b6e593a>`: 551/8 — trzy asercje 3b (12 kopii,
  brak biblioteki, `do_pair` bez argumentu), `noeval` (to drzewo jest sprzed
  PR #299) i cztery asercje ścieżek profilu/`$SCRIPT_DIR`, które padają dla
  każdego obcego drzewa.

- **Ostatnie cztery trywialne `eval` poza silnikami usunięte (2026-09-03,
  punkt 3 z listy redukcji).** `lib-cron.sh` zamykało deskryptor zamka
  crontaba przez `eval "exec $fd>&-"` w dwóch miejscach (odmowa po
  przekroczeniu `flock -w` i `cron_lock_release`); bash zamyka deskryptor o
  numerze trzymanym w zmiennej wprost, tą samą formą `{fd}>&-`, która go
  otworzyła. `zfs-backup.sh` czytało numerowane pola wykluczeń rekordu klienta
  (`EXCLUDE_FAMILY_n`, `EXCLUDE_CHILD_n`) przez `eval "v=\${..._$i:-}"` w
  `client_passive_flags`/`client_exclude_flags`; teraz `n=EXCLUDE_CHILD_$i;
  v=${!n:-}` — zmienia się tylko numer, a nazwa nie jest poleceniem. Nic
  obserwowalnego się nie zmienia (wartość nigdy nie była ponownie parsowana w
  żadnej z form), więc dyskryminatorem jest sam mechanizm: licznik `eval` w
  obu plikach, czerwony na `main` (2 i 2). Suity: `cron` sekcja S (+6: licznik,
  deskryptor otwarty przy trzymanym zamku, zamknięty po zwolnieniu, wpis
  księgowy usunięty, odmówione nabycie nie zostawia deskryptora — liczone przez
  `/proc/$$/fd`), `zfsbackup` sekcja `noeval` (+4, `--section noeval`:
  licznik, pola czytane po kolei do pierwszej luki, wartość w kształcie
  polecenia zostaje tekstem — sonda dowodzi, że renderowanie w ogóle
  zaszło, zanim uzna brak markera za wynik, oraz puste pola pod `set -u`).
  Kontrole ujemne wobec `main` (`b6e593a`): `cron` z `LIB=<main>` 139/1 (tylko
  S1), `zfsbackup --section noeval` z `ZFSBACKUP=<main>` — czerwony licznik
  plus trzy asercje sekcji 96, które porównują ścieżkę profilu z drzewem suity
  i padają dla KAŻDEGO obcego drzewa, nie z powodu tej zmiany. W silnikach
  zostaje sześć `eval` (`$dry`, `$canmount_cmd`, pomiar pipeline'u w
  `lib-zfs-snap.sh`) — wymagają dowodu na żywym ZFS. Dwie wady sond złapane
  przed commitem, zapisane jako E28/E29 w `IMPLEMENTER-ERROR-LOG.md`.
...
- **Mapa redukcji domknięta: reszta to duplikacja ŚWIADOMA, brakowało jej opisu (2026-08-20).**
  ...
  **`config_datasets`** — bez callera produkcyjnego po usunięciu migrate;
  wtedy zostawiona jako „przetestowana definicja konwencji", którą planer
  `assert_no_overlapping_policy` miał powtarzać inline. **Usunięta 2026-09-03**:
  planer o tej nazwie nie istniał w żadnej rewizji repo, a jedna gramatyka listy
  to `dataset_list_split` w `lib-scope.sh`, pilnowana w `test/scope` sekcja D
  (przecinki, białe znaki, puste pozycje). Kopia bez callera to nie rezerwa,
  tylko drugi egzemplarz czekający na rozjazd — `test/deadcode` (niżej).
...
- Data odświeżenia: **2026-08-18**. Ostatnia zmiana zachowania: **konto zadań
  ...
  **Werdyktu „nic do zrobienia" już nie ma**: `written=0` odzwierciedla ostatni
  ...
  Ścieżka niszcząca to silnik `restore_replace_pair`, osiągalny przez publiczny
  czasownik (`restore_run_scope` → `restore_one`; do 2026-09-03 obok siedział
  starszy wrapper `restore_replace_internal` bez callera — usunięty, bateria
  `test/restore` woła silnik wprost). Silnik: rozwiązuje relację, odrzuca
  ...
  **CO JEST WDROŻONE, A CO NIE — stan na dziś.** To jest jedyne miejsce, gdzie
  ...
  | restore | **WDROŻONE przez publiczne CLI (`zfs-restore.sh`, w checkoucie każdego hosta), bez stałych grantów** (Faza 7, stan 2026-09-03): planer read-only (`--plan`), ścieżka BEZPIECZNA LIVE-PROVEN 2026-08-15 (md5+GUID), ścieżka NISZCZĄCA za grantem na maszynie zagrożonej (`deploy.sh --allow-restore=<label>`, `--replace` gdy niszczy dane, które ta maszyna trzyma), potwierdzeniem zmierzonego zbioru strat (snapshot techniczny, bookmarki po `createtxg`, rewalidacja tuż przed komendą niszczącą, `zfs destroy` jawnie + rollback NIErekurencyjny — REV-119/120/121), ogrodzeniem zapisu i werdyktem per dataset (D+B, REV-127). Gramatyka publiczna ustalona i wdrożona: nazwa relacji sama, `--target` po przecinkach, `--at` po `creation`, drugi adres = inna maszyna, `--onto` = gdzie ląduje (dwukropek wycofany, #132), `--from-copy` gdy rekordów relacji nie ma. Dowody na żywo: trzy hosty 2026-08-28, drugi przebieg labu 2026-08-27, pełny dziesięciokrokowy lab 2026-08-31 @ `bb5b6af` bez poprawki kodu. Wszystkie REV Fazy 7 ZAMKNIĘTE. **Gate 7 ZADEKLAROWANY przez właściciela 2026-09-03** na faktach zestawionych w `ACTIVE-WORK-PLAN.md` (sekcja „Gate 7 — what the facts said"), REACHED | wiersz suity `restore` niżej; `OWNER-RESTORE-CLI-GRAMMAR-2026-08-13.md`, `OWNER-RESTORE-SCOPE-2026-08-26.md`, `OWNER-RESTORE-GRANT-AND-MODES-2026-08-26.md`, `OWNER-RESTORE-CROSS-HOST-GRAMMAR-2026-08-30.md` |
...
| `tune` | **56/56** | cache autotune `-A`; sonda `tune_probe_stream` — od 2026-09-03 snippet po stronie danych woła funkcję `h()` zamiast `eval`-ować łańcuch trzy razy (odmrożenie właściciela, `test/evalfree` pinuje kształt) |
| `evalfree` | **16/16** (nowa, 2026-09-03) | zero `eval` w pięciu zamrożonych plikach (licznik słowa poza komentarzami) i dowód, że trzy mechanizmy, które je zastąpiły, robią to samo: `announce_transfer_size` pod pseudo-tty na stubie `zfs` zapisującym argv — send z `$(touch MARKER)` daje zapowiedź, dosłowny tekst w argv i BRAK markera (na `main` marker powstaje); para `canmount_noauto_subtree`/`_cmd` daje identyczne `zfs set` na liście z nazwą ze spacją, a tekst jest bajt w bajt starym one-linerem; snippet sondy ma `h()`. Kontrola ujemna wobec `main`: licznik, marker/argv i FATAL na braku helpera |
...
| `runsuffix` | **15/15** | jeden sufiks nazwy snapshotu na PRZEBIEG, nie na dataset (Etap 2.1). Własność, od której zależy restore: zestawu snapshotów, którego nie da się zidentyfikować jako jednego przebiegu, nie da się odtworzyć jako jednego. `create_snapshot` wyekstrahowane z OBU silników, `date(1)` zaślepione tak, by zwracało INNĄ wartość przy każdym wywołaniu — dokładnie to, co robi prawdziwe poddrzewo przekraczające granicę sekundy. Przypina też KSZTAŁT nazwy, bo zależą od niego wzorce `delsnaps`, prefiksy monitora i każda zainstalowana linia crona. Licznik zaślepki żyje w PLIKU, nie w zmiennej: `$(date ...)` biegnie w podpowłoce, więc licznik na zmiennej zwracałby tę samą wartość i kontrola negatywna przeszłaby na STARYM kodzie, nie dowodząc niczego (pierwsza wersja tego testu robiła dokładnie to). Kontrola negatywna wobec `643238a`: **2 przypadki korelacji padają, 4 nietknięte przechodzą**. Korelacja end-to-end na prawdziwym ZFS-ie należy do `test/scenarios`; ta suita przypina samą decyzję o nazywaniu |
...
- **Znane luki produktowe z sierpnia — ZMIERZONE 2026-09-03 (to nie jest stan
  recenzji).** Cztery pozycje nazwane w odpowiedziach z początku sierpnia niosły
  do dziś adnotację „nie weryfikowane ponownie". Pomiar na `main` `5b9e097`:
  (1) `cron2conf.sh` **nie parsowało linii `snapget.sh`** — parser przyjmował
  tylko `snapsend.sh -m "…"` i `delsnaps.sh`, a pierwsza linia pull kończyła
  bieg `die "unrecognized job line"`, więc crontab kolektora był
  nieodtwarzalny w całości; **NAPRAWIONE** tego dnia (`parse_get_cmd`, sekcje
  `[dataset:<baza>/<nazwa zdalna>]` z `src`, `autotune = no` gdy w linii
  zdalnej brak `-A`; fixture `pull.crontab` z sześcioma kształtami pull, które
  ...
  parowania, czyli labu — **ZAMKNIĘTA 2026-09-03/04**: lab
  `docs/discussions/LAB-DRAFT-CONFIG-2026-09-03.md` wykonany na pve9↔pve10,
  wynik w `LAB-DRAFT-CONFIG-WYNIK-2026-09-03.md` (PR #317): sześć własności
  potwierdzonych; komenda nigdy nieużyta na siedmiu hostach, ale wykonawca
  rekomenduje ZOSTAWIĆ (ścieżka niskopoziomowa `--pair`→wsad→`--join` jest w
  realnym użyciu, gdy `--join-remotely` nie pasuje). Wszystkie cztery luki z
  sierpnia zmierzone. (3) silniki **odrzucają `-r -R` naraz**:
  ...
  liczony raz na przebieg na poziomie pliku, suita `runsuffix` pinuje „jeden
  sufiks na PRZEBIEG" (Etap 2.1) — ZAMKNIĘTA. Zostaje więc jedna luka, (2),
  i jest labowa, nie kodowa.
...
2. Implementer implementuje i testuje na gałęzi roli i dostarcza przez Pull
   Request; scalenie na `main` jest ruchem właściciela („Scal PR #N"). Tymczasowy
   wyjątek na pracę wprost w `main` właściciel cofnął 2026-08-14.

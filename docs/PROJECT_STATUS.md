...
| `pairpause` | **85/85** | pauza logiczna jednej relacji (REV-20260804-045): walidator etykiety wraz z przypadkami traversalu (`..`, `a/b`), `pair_is_paused` fail-OPEN (brak drzewa stanu NIE może czytać się jako „wszystko wstrzymane”), `pause-client`/`resume-client` — jeden marker zapisany atomowym rename, idempotencja w obie strony, odmowa bez ŻADNEJ mutacji FS przy złej i nieznanej etykiecie, izolacja dwóch relacji. Rdzeń: zapauzowany przebieg `snapsend.sh`/`snapget.sh` kończy się NIE wywoławszy `zfs`, `ssh`, `mbuffer`, kompresora ani nawet `flock` — dowodzone pustym plikiem trace ze stubów w `PATH`, nie czytaniem kodu. Osobno asercja UDOKUMENTOWANEGO OBEJŚCIA: wywołanie bez `--pair-label` NIE jest blokowane — to nie przeoczenie do cichego naprawienia, tylko granica między pauzą logiczną a niezaimplementowanym hard disable. Kontrakt `pair-pause-state` (writer w `zfs-backup.sh`, reader w `lib-zfs-snap.sh`, zero wspólnego kodu) sprawdzany przez porównanie domyślnych ścieżek i obu walidatorów |

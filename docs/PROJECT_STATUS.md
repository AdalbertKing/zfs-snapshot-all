...
<!-- status-covers-digest: 265bd2961874cd84 -->
...
- Data odświeżenia: **2026-08-09**. Stan plików, który ten blok opisuje, jest
  ...
  **CO JEST WDROŻONE, A CO NIE — stan na dziś.** To jest jedyne miejsce, gdzie
  ...
  | Etap 5 — RUNTIME profilu (`zfs-backup.sh` czyta profil zamiast zaszytych szablonów) | **wdrożony** (B1 krok 2): `ensure_cron_config` i `emit_client_sections` czytają wyrenderowany profil, pre-GFS **odmawia** zamiast konwertować; suita `zfsbackup` 314/314, kontrola negatywna 14 pada | `docs/design/slice-b1-plan.md` |
  ...
  | preflight nakładania pokrycia (create-only preset odmawia relacji, której ścieżka jest rodzicem/dzieckiem/dokładnym trafieniem cudzego pokrycia; fail-closed też na rekordzie nie do odczytu/parsowania; sprawdzane PRZED prawdziwym `seed`, nie tylko przy `activate-client`) | **wdrożony**, kod: `coverage_conflicts`/`assert_no_coverage_overlap` w `zfs-backup.sh`, wywoływany z `cmd_seed()` i `emit_client_sections()`; suita `zfsbackup` sekcje 45/46/47 | REV-20260809-083/084/085; wymagany dowód na żywo (odmowa PRZED jakąkolwiek nową mutacją po stronie odbioru + nadpisanie CONFIG/crontaba na prawdziwym hoście) NIEwykonany, patrz odpowiedzi REV-083/084/085 |

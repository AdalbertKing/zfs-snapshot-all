...
<!-- status-covers-digest: 81047e5e0a5f818d -->
...
- Data odświeżenia: **2026-08-13**. Ostatnia zmiana zachowania: `eb7cf11`
  (2026-08-13, bramki ścieżki niszczącej — wewnętrznie, bez kroku wykonawczego
  i bez publicznej flagi). Faza 7 ma na main wycinek 1 (planer read-only), wycinek 2
  ...
  **Werdyktu „nic do zrobienia" już nie ma**: `written=0` odzwierciedla ostatni
  ...
  Bramki ścieżki niszczącej **istnieją jako funkcja WEWNĘTRZNA**
  (`restore_replace_internal`, nieosiągalna z CLI): rozwiązuje relację, odrzuca
  wszystko, czego nie umie dowieść, pokazuje ten sam podgląd co `--plan` i staje
  przed krokiem wykonawczym. **Nic niszczącego nie jest zaimplementowane** —
  łącznie ze snapshotem przed zniszczeniem, który należy do kroku wykonawczego.
  **Publicznej gramatyki nie ma i celowo nie powstaje**: właściciel dopiero
  ...
  **CO JEST WDROŻONE, A CO NIE — stan na dziś.** To jest jedyne miejsce, gdzie
  ...
  | restore | **CZĘŚCIOWO wdrożone** (Faza 7): planer read-only, bezpieczne odtworzenie do wyprowadzonego namespace, mapa planera i podgląd domyślnej strategii odtworzenia. Ścieżka NISZCZĄCA istnieje jako **same bramki, wewnętrznie** — odmawia wszystkiego czego nie dowiedzie, pokazuje podgląd i staje; kroku wykonawczego nie ma, publicznej gramatyki też nie (właściciel ją właśnie ustala) | wiersz suity `restore` niżej; `OWNER-RECOVERY-DEFAULT-POLICY-2026-08-12.md` |

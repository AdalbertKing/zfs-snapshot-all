...
| 21c | Fleet migration to `recursive` | `192.168.11.11` is the one managed config with `-r` in `flags`. One-line edit, proved byte-identical live. Not applied — it is a production file. Nothing regenerates crontabs on a schedule (checked on all four hosts), so no host changes behaviour; the refusal only appears on a manual `gen-cron.sh`. | owner: apply it, or say when |

...
#   zfs-backup.sh restore --replace --dataset=DATASET [--config=FILE]
...
Usage:
  ...
  zfs-backup.sh restore --replace --dataset=DATASET [--config=FILE]
                                    DESTRUCTIVE recovery of the original path to the latest
                                    valid backup (owner default policy). Currently previews
                                    and REFUSES: every gate exists, the execution step does
                                    not. No --snapshot: the recovery point is the latest
                                    valid backup, by policy.
  zfs-backup.sh add-client NAME --lan=HOST[:PORT] (--datasets="A B" | --mode=backup|sync) [--target=X] [--bandwidth=N] [--profile=NAME] [--join-remotely]
  zfs-backup.sh seed NAME [--yes]
...
# this and `--replace` resolves a single dataset through it; a second copy of the
...
# Phase 7 slice 3 (slice 1 of the destructive path) -- `restore --replace`.
#
# The owner's default recovery is "latest valid backup -> the original source
# path", which means destroying source state. This slice builds every gate in
# front of that and NOT the step itself: it resolves the relationship, refuses
# everything it cannot prove it understands, prints the same preview `--plan`
# prints, and stops.
#
...
# with the execution it guards, in the next slice.
#
# Nothing here mutates anything -- not even the pre-destruction snapshot, which
# belongs to the execution step. A verb that snapshotted and then refused would
# leave litter behind on every attempt.
cmd_restore_replace() {   # <dataset> <config> <yes>
    local dataset="$1" config="$2" yes="$3"
    [ -n "$dataset" ] || die "restore --replace: nazwij co odtwarzac -- --dataset=<sciezka zrodla albo kopii>. Bez tego nie ma pytania."

    read_server_conf
    [ -n "$config" ] || config="${CRON_CONFIG:-}"
    [ -n "$config" ] || die "restore --replace: no cron config known -- pass --config=FILE or run setup-server"
    [ -r "$config" ] || die "restore --replace: cannot read $config"
    ...
    [ "$hits" -ne 0 ] || die "restore --replace: '$dataset' nie wystepuje w zadnej relacji backupu w $config -- 'restore --plan' pokaze te, ktore istnieja"
    ...
    [ "$hits" -eq 1 ] || die "restore --replace: '$dataset' pasuje do $hits relacji w $config -- nie zgaduje ktora; nazwij dokladny dataset zrodla albo kopii"
    ...
    [ "$cons" != atomic ] || die "restore --replace: '$src' jest w relacji ATOMIC (spojne poddrzewo w jednym punkcie czasu), a ten czasownik odtwarza pojedynczy dataset. Odtworzenie tylko jego zlamaloby wlasnosc, dla ktorej ta relacja jest atomowa. Odtwarzanie poddrzew nie istnieje."

    local snaps
    snaps=$(zfs list -H -p -t snapshot -o name,creation,guid -s creation -d 1 "$copy" 2>/dev/null) || snaps=""
    [ -n "$snaps" ] || die "restore --replace: kopia '$copy' nie ma ani jednego snapshota, wiec nie ma z czego odtwarzac"
    ...
    case "$RESTORE_STRATEGY" in
        remote)
            die "restore --replace: zrodlo '$src' jest ZDALNE. Ten czasownik dziala tylko lokalnie -- odtworzenie na zdalny host wymaga decyzji o tym, kto wykonuje zniszczenie po tamtej stronie, a tej decyzji nie ma." ;;
        full-absent|full-live)
            die "restore --replace: nie ma wspolnej bazy dowiedzionej GUID-em, wiec jedyna droga jest PELNE zastapienie -- inny mechanizm i inne ryzyko niz przyrost. Nie istnieje w tym wycinku i nie bedzie udawane przyrostem." ;;
        increment|rollback|discard-live|unproven) ;;
        *)  die "restore --replace: nie ustalono strategii dla '$src' -- odmawiam dzialania na nieokreslonym stanie" ;;
    esac

    die "restore --replace: krok WYKONAWCZY jeszcze nie istnieje (nastepny wycinek). Powyzszy podglad jest kompletny i policzony na zywym stanie, ale nic nie zostalo zrobione: nie powstal zaden snapshot, nic nie zniszczono, '$src' jest dokladnie w stanie sprzed tego polecenia."
}

cmd_restore() {
    local plan=0 replace=0 dataset="" config="" snapshot="" yes=0
    for a in "$@"; do
        case "$a" in
            --plan)       plan=1 ;;
            --replace)    replace=1 ;;
    ...
    # --replace is its own verb spelled as a flag, and it is NOT a --force on the
    # safe path: it takes no --snapshot, because the owner's default recovery point
    # is the latest valid backup and choosing a different one is a different
    # conversation.
    if [ "$replace" -eq 1 ]; then
        [ "$plan" -eq 0 ] || die "restore: --plan and --replace are different verbs; --replace previews before it does anything, so asking for both is asking for two answers to one question"
        [ -z "$snapshot" ] || die "restore --replace: --snapshot is not accepted. The recovery point is the latest valid backup, by owner policy. Use --plan to see the points that exist."
        cmd_restore_replace "$dataset" "$config" "$yes"
        return
    fi
    ...
    if [ "$plan" -ne 1 ]; then
        [ -n "$snapshot" ] || die "restore: give --plan to see what could be restored, --snapshot=NAME (with --dataset=) to restore one safely into the restore namespace, or --replace --dataset=NAME for the destructive recovery of the original path."
        cmd_restore_safe "$dataset" "$snapshot" "$config" "$yes"
        return
    fi

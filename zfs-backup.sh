...
# thing that turned the zfsbackup test leg into 34 minutes. One ceiling for every
# call site; env-overridable, and deliberately not readonly (this file is
# sourced, sometimes more than once in a shell).
...
check_inherited_grants() {
    ...
    local -a opts=(-i "$keyfile" -p "$port" -o BatchMode=yes -o "HostKeyAlias=$alias" -o UserKnownHostsFile="$alias_kh" -o StrictHostKeyChecking=yes -o GlobalKnownHostsFile=/dev/null -o CheckHostIP=no -o ConnectTimeout="$SSH_CONNECT_TIMEOUT")
...
assert_source_prune_grant() {   # <account> <host> <port> <keyfile> <alias> <alias_kh> <source-dataset>...
    ...
    local -a opts=(-i "$keyfile" -p "$port" -o BatchMode=yes -o "HostKeyAlias=$alias" \
        ...
        -o GlobalKnownHostsFile=/dev/null -o CheckHostIP=no -o ConnectTimeout="$SSH_CONNECT_TIMEOUT")
    local ds out rc
    for ds in "$@"; do
    ...
    if [ "$sync_mode" -eq 1 ]; then
        ...
        for pds in $PEER_SAVED_DATASETS; do
            if ssh -i "$LOAD_KEYFILE" -p "$LOAD_PORT" -o BatchMode=yes \
                   ...
                   -o StrictHostKeyChecking=yes -o GlobalKnownHostsFile=/dev/null -o CheckHostIP=no -o ConnectTimeout="$SSH_CONNECT_TIMEOUT" \
...
pair_control() {   # <verb> -> prints the gate's reply, non-zero on failure
    ...
    local -a ssh_opts=(-i "$LOAD_KEYFILE" -p "$LOAD_PORT" -o BatchMode=yes \
        ...
        -o CheckHostIP=no -o ConnectTimeout="$SSH_CONNECT_TIMEOUT")
...
rux_root_ssh() {   # <host> <port> <command...>
    ...
    ssh -o BatchMode=yes -o UserKnownHostsFile=/root/.ssh/known_hosts \
        -o StrictHostKeyChecking=yes -o ConnectTimeout="$SSH_CONNECT_TIMEOUT" \
        -p "${port:-22}" "root@$host" "$@"
}

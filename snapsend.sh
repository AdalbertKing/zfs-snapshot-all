...
process_dataset() {
    ...
    if [ $FORCE_FULL_SEND -ne 1 ]; then
        ...
        # Still ONE ssh round trip: the remote side reports which branch it
        # took rather than being asked twice.
        local _tgt_state=""
        if [ -n "$remote_host" ]; then
            _tgt_state=$(ssh -n "${SSH_OPTS[@]}" "$remote_user@$remote_host" \
                "if zfs list '$create_target' >/dev/null 2>&1; then echo have; else zfs create -o canmount=$TARGET_CANMOUNT '$create_target' && echo made; fi") || {
...
# the one `getopts` actually parses with was a separate literal a hundred lines
# below, and when -E (the excluded snapshot family) was added there it was not
# added here. So the pre-pass did not know -E takes an argument, read the family
# NAME as the first non-option argument, and stopped processing options at it:
#

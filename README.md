...
- [Core concepts](#core-concepts)
  ...
  - [Pausing one relationship (`--pair-label`)](#pausing-one-relationship---pair-label)
...
### Pausing one relationship (`--pair-label`)

`--pair-label <LABEL>` tags a job with the **relationship** it belongs to — the client label
recorded at enrolment by `zfs-backup.sh add-client`. Jobs generated by `activate-client` carry it
automatically; you rarely type it by hand.

Its only purpose is to make one relationship pausable without touching anything else:

```bash
zfs-backup.sh pause-client  pve2 --reason="okno serwisowe"
zfs-backup.sh status        pve2      # Pauza: PAUSED_LOCAL od ...
zfs-backup.sh resume-client pve2
```

`pause-client` writes exactly one marker file under `/etc/zfs-snapshot-all/relationships/<LABEL>/`.
It does **not** edit cron, the generated job config, `zfs allow`, or `authorized_keys` — the
installed cron lines stay byte-identical and simply start skipping. Every dataset and every
endpoint candidate of one relationship observes one state, because the key is the durable label,
not a host or an IP. A paused run exits **0** having created no snapshot, taken no hold and opened
no connection, and records `SKIPPED_PAUSED` in the [stats log](#json-lines-stats-log) — the exit
code deliberately stays 0 so an intentional pause does not page anyone, and the status field, not
the exit code, is what tells it apart from a real successful transfer.

> **This is orchestration, not a security boundary — and the difference matters.**
> The gate lives on this host and keys on a flag the *caller* passes. A hand-written
> `snapget.sh`/`snapsend.sh` that omits `--pair-label` is **not** blocked, and neither is raw
> `ssh` with the relationship's key. Refusing the data plane no matter what the caller passes
> needs a gate on the *peer*, which authenticates the key and can therefore identify the
> relationship on its own. That is not implemented. Do not use `pause-client` where you need
> "this peer cannot reach us"; use it where you need "stop these jobs for a while".

One consequence worth knowing before a long pause: **retention keeps running while paused.**
`delsnaps.sh` jobs are not gated, so the source may prune past the last replicated snapshot. The
[bookmark fallback](#bookmark-backed-incremental-fallback) is what saves the incremental in that
case — so do not also prune the per-target bookmarks (`delsnaps.sh -B`) on a relationship you have
paused, or resuming will need a full re-seed.
...
./test/pairpause/run.sh # --pair-label / pause-client: the gate, and its documented bypass
```

These five need **no root, no ZFS, no network** — plain bash + coreutils, so they run the same
